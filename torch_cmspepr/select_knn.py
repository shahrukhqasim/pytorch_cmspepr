...
def select_knn_directional(
    ...
    if batch_x is None:
        ...
        row_splits_x: torch.Tensor = torch.tensor(
            [0, x.shape[0]], dtype=torch.int32, device=x.device
        )
        row_splits_y: torch.Tensor = torch.tensor(
            [0, y.shape[0]], dtype=torch.int32, device=y.device
        )
    else:
        ...
        assert batch_size == int(torch.max(batch_y)) + 1
        ...
        row_splits_x = torch.zeros(batch_size + 1, dtype=torch.int32, device=x.device)
        torch.cumsum(counts, 0, out=row_splits_x[1:])


        counts = torch.zeros(batch_size, dtype=torch.int32, device=x.device)
        counts.scatter_add_(0, batch_y, torch.ones_like(batch_y, dtype=torch.int32))
        row_splits_y = torch.zeros(batch_size + 1, dtype=torch.int32, device=x.device)
        torch.cumsum(counts, 0, out=row_splits_y[1:])
        print(row_splits_x)
        0/0
    ...
    if x.device == torch.device('cpu'):
        ...
        ret = torch.ops.select_knn_cuda.select_knn_directional_cuda(
            x,
            row_splits_x,
            y,
            row_splits_y,
